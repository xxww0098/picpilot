...
func TestDoJSONImagesGenerationsUsesOutputItemDoneWhenCompletedResponseOmitsOutput(t *testing.T) {
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != codexResponsesPath {
			t.Fatalf("unexpected path=%q", r.URL.Path)
		}
		w.Header().Set("Content-Type", "text/event-stream")
		_, _ = io.WriteString(w, `data: {"type":"response.output_item.done","item":{"type":"image_generation_call","result":"aW1n","revised_prompt":"cat"}}`+"\n\n")
