...
export async function callFalAiImageApi(opts: CallApiOptions, profile: ApiProfile): Promise<CallApiResult> {
  try {
    ...
    logger.info('api', '← fal.subscribe 成功', { endpoint, requestId: result.requestId, elapsedMs: Date.now() - startedAt })
    ...
    logger.warn('api', 'fal.subscribe 失败', { error: serializeError(err), resolvedMessage: falMessage ?? undefined })
