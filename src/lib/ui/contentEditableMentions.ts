...
export function getMentionTagTextLength(el: Element) {
  return el.textContent?.length ?? 0
}

export function getNodeVisibleTextLength(node: Node): number {
...
export function getVisibleOffsetBeforeNode(root: HTMLElement, target: Node): number {
  let offset = 0
  let found = false
...
export function getMentionTagForBoundary(root: HTMLElement, container: Node) {
...
export function getBoundaryOffsetInMention(tag: Element, container: Node, offset: number) {
  try {
...
export function getContentEditableBoundaryOffset(
  root: HTMLElement,
  container: Node,
  offset: number,
  edge: 'start' | 'end',
