...
export function getUserFacingErrorMessage(error: unknown, fallback = '操作失败', httpStatus?: number): string {
  ...
  if (/用户名或密码错误|登录失败次数过多|邀请码|密码至少|用户名长度|用户名已被占用|额度|团队服务|批量生成数量上限/.test(message)) {
    return message
  }

  if (/API 代理未配置上游地址/.test(message)) {
    return '团队 API 代理还没有配置上游地址，请管理员检查部署环境中的 TEAM_API_BASE_URL 或 API_PROXY_URL。'
  }
  ...
  if (/unauthorized|invalid authentication|invalid[\s_-]*api[\s_-]*key|missing[\s_-]*api[\s_-]*key|401/i.test(message)) {
    return '认证失败：API Key 无效、已过期或没有传到上游。请检查当前配置或联系管理员确认团队默认 Key。'
  }

  if (/forbidden|403/i.test(message)) {
    return '权限不足：当前账号或 API Key 没有执行此操作的权限。'
  }

  if (/rate.?limit|too many requests|429/i.test(message)) {
    return '请求被限制：额度已用完或请求过于频繁，请稍后再试。'
  }
  ...
  if (httpStatus === 429) return '请求过于频繁或额度已用完，请稍后再试。'
