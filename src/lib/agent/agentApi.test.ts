...
import { createDefaultOpenAIProfile, DEFAULT_RESPONSES_MODEL, DEFAULT_SETTINGS } from '../shared/apiProfiles'
...
describe('callAgentResponsesApi', () => {
  ...
  it('uses the API upstream mode by default', async () => {
    ...
    await callAgentResponsesApi({
      settings: DEFAULT_SETTINGS,
      profile: createDefaultOpenAIProfile({ apiMode: 'responses' }),
      params: DEFAULT_PARAMS,
      input: [{ role: 'user', content: [{ type: 'input_text', text: 'prompt' }] }],
    })
