...
// 识别 Cloudflare 挑战/拦截页（直连或 cliproxy 包成 JSON 的都覆盖）。标记与后端
// server-go 的 isCloudflareManagedChallenge 保持一致，避免 CF HTML 里夹带的
// "401"/"403" 把错误误判成认证/权限问题。
function isCloudflareChallengeMessage(message: string): boolean {
  return /cloudflare|cdn-cgi|cf[_-]chl|cf-mitigated|challenge-error-text|challenge-platform|just a moment|enable javascript and cookies to continue|attention required|you have been blocked|ray\s*id/i.test(
    message,
  )
}
...
export function getUserFacingErrorMessage(error: unknown, fallback = '操作失败', httpStatusOrOpts?: number | GetUserFacingErrorMessageOptions): string {
  ...
  // CF 挑战/拦截页（HTML）常夹带 "401"/"403" 等子串，必须在认证/权限正则之前拦下，
  // 否则会被误判成"认证失败，请检查 API_PROXY_API_KEY"（Key 其实没问题）。Agent 路径
  // 不带 apiUpstream 标记，也要走这条判断。
  if (isCloudflareChallengeMessage(message)) {
    return '上游服务暂时不可用（Cloudflare 网关拦截），请稍等几分钟后重试。'
  }

  // 上游 API 错误已由后端原样透传，直接展示给用户，不做正则改写。
  if (apiUpstream) {
    // 后端透传的 5xx 网关错误 → 给出明确的等待提示。
  ...
  // CPA 逆向账号导入：避免把管理接口 401 误判成 API_PROXY_API_KEY 问题。
  if (/无法连接 CPA|CPA 管理令牌|CPA 管理接口|CLIProxyAPI 账号列表|连接 CLIProxyAPI 失败/.test(message)) {
    return message
  }
  ...
  if (/failed to fetch|fetch failed|networkerror|network request failed|load failed|\bnetwork error\b/i.test(message)) {
    return preserveHint(
      message,
