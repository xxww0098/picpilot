...
- **镜像本机构建**(非拉取 ghcr):`/opt/picpilot/compose.yml` 用 `build:` 从源码构建 `picpilot-frontend`(`deploy/Dockerfile`)与 `picpilot-auth`(`server/Dockerfile`);镜像标签 = `PICPILOT_VERSION`,**不带 `v` 前缀**(如 `0.1.7`,与历史 `0.1.6` 一致)。
