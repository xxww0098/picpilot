...
- **镜像本机构建**(非拉取 ghcr):`/opt/picpilot/compose.yml` 用 `build:` 从源码构建 `picpilot-frontend`(`deploy/Dockerfile`)与 `picpilot-auth`(`server-go/Dockerfile`);镜像标签 = `PICPILOT_VERSION`,**不带 `v` 前缀**(如 `0.1.7`,与历史 `0.1.6` 一致)。
- **服务**(compose):`caddy`(TLS 反代)、`cliproxy`(上游出图代理)、`dockercopilot`、`frontend`、`auth`。容器名形如 `picpilot-auth-1`。
...
# 2) 后端已监听（确认 server/index.ts 的 Node 启动守卫在生产生效）
docker logs picpilot-auth-1 --tail 30 | grep "picpilot ready"
...
- **日志**:auth `docker logs picpilot-auth-1`(可加 `-f`/`--tail 200`/`--since 1h`);前端 `docker logs picpilot-frontend-1`;cliproxy 出图路由与耗时见 `/opt/picpilot/data/cliproxy/logs/main.log`(`error-*.log` 为单请求错误快照);caddy `docker logs picpilot-caddy-1`。
- **后端 Dockerfile**:`server-go/Dockerfile` 构建 CGO-free Go 二进制。新增 Go 文件或目录只要在 `server-go/` context 内会被 `COPY . .` 纳入镜像。
- **lockfile**:两个 Dockerfile 用 `npm ci`,改 `package.json` 依赖后务必同时提交对应 `package-lock.json`,否则构建失败。
