...
- **平台化工作流模板**：工作流画布按平台拆分上架流程，不再用笼统“电商”模板混用规则。当前内置 Ozon 上架图组（3:4 主图、Fresh 方图、场景附图、细节附图、信息图，含示例商品图）与独立站详情页复刻等模板。
...
- **团队 API 代理**：所有 OpenAI 兼容请求一律走同源 `/api-proxy/` 路径，由服务端按当前登录用户校验后转发到当前上游。支持 `api` 与 `reverse` 两种模式，前端不暴露真实上游地址与 Key。
...
Docker 部署建议把上游凭据放在服务端环境变量里。PicPilot 是**单容器**（`auth`，Go 后端 + 前端静态文件打在同一个镜像里）；如果同一台服务器上还部署 CLIProxyAPI，推荐把它们放进同一个 Docker Compose 网络，由 `auth` 通过服务名访问 CLIProxyAPI。

**环境变量说明：**

> 变量模板见 [`deploy/vps/.env.example`](deploy/vps/.env.example)，下面按重要性分组列出。
...
| `UPSTREAM_MODE` | 上游模式：`api` 或 `reverse` | `api` |
| `API_PROXY_URL` | API 模式上游地址，需写到 `/v1` | `http://cli-proxy-api:8317/v1` |
| `API_PROXY_API_KEY` | API 模式上游 Key | `your-api-key` |
| `REVERSE_PROXY_URL` | reverse 模式地址；`internal` 表示 Go 内置 ChatGPT 逆向 | `internal` |
...
| `OUTBOUND_PROXY_TYPE` | 服务端出站代理默认类型：`env` / `none` / `http` / `https` / `socks5` / `socks5h`，可在管理端运行时覆盖 | `env` |
| `OUTBOUND_PROXY_URL` | 出站代理默认地址，格式 `host:port` 或完整代理 URL | 空 |
| `CHATGPT_REVERSE_ACCOUNT_CONCURRENCY` | 内置 reverse 每个账号同时执行的请求数；单 IP 账号池建议 `1`，可在管理端运行时覆盖 | `1` |
...
> 💡 **上游地址与 Key 不暴露给前端**：前端用户不能也不需要在设置里填 `base_url` 或 `API Key` —— 所有 OpenAI 兼容请求一律走同源 `/api-proxy/...`。`UPSTREAM_MODE=api` 时服务端读 `API_PROXY_URL`/`API_PROXY_API_KEY`；`UPSTREAM_MODE=reverse` 且 `REVERSE_PROXY_URL=internal` 时走 Go 内置 ChatGPT 逆向，ChatGPT OAuth JSON 由管理员在「管理面板 → 逆向账号」导入并存入 SQLite 数据库。

> 💡 **`/v1` 路径规则**：`API_PROXY_URL` 与外部 `REVERSE_PROXY_URL` 都推荐写到 `/v1`。服务端会容忍外部请求路径里也带 `/v1` 的情况，例如 `/api-proxy/v1/models` 会转成上游 `/v1/models`，不会拼成 `/v1/v1/models`。内置 reverse 使用 `REVERSE_PROXY_URL=internal`，不需要 `/v1`。

> 💡 **Go 内置 reverse 模式**：设置 `UPSTREAM_MODE=reverse`、`REVERSE_PROXY_URL=internal`，管理员在「管理面板 → 逆向账号」导入 ChatGPT/Codex OAuth JSON。账号只存入 PicPilot SQLite 数据库，不需要给 auth 容器挂载账号目录；如仍需外部 chatgpt2api，可把 `REVERSE_PROXY_URL` 改回它的 `/v1` 地址并填写 Bearer key。单 IP 账号池的保守最佳值是每账号 `1` 并发，可在「管理面板 → 团队设置 → 逆向单账号并发」调整到 `1-5`。
...
# 创建环境变量
cp deploy/vps/.env.example deploy/.env
# 编辑 deploy/.env，填入 JWT_SECRET、ADMIN_USERS，并按 UPSTREAM_MODE 填对应上游变量
...
services:
  ...
  caddy:
    ...
    command: ["caddy", "reverse-proxy", "--from", "${PUBLIC_ORIGIN:-localhost}", "--to", "auth:3001"]
    ports:
      - "80:80"
      - "443:443"
    volumes:
  ...
  # PicPilot 单容器：Go 后端 + 前端静态文件（构建上下文是仓库根）
  auth:
    build:
      context: .
...
如果返回的 `upstreamMode` 与 `.env` 一致且 `upstreamConfigured` 为 `true`，说明 Go 后端已读取到对应上游配置。

</details>

<details>
<summary><strong>💻 方式二：本地开发与静态构建</strong></summary>
...
`start:local` 会构建前端产物，然后用 `go run` 启动 Go 后端（需本机安装 Go 1.26+）：Go 服务同时负责 `/api/*` 接口和 `dist/` 静态文件，因此访问 `http://localhost:3001` 会显示登录界面。默认本地管理员为 `admin` / `admin`；如需自定义，可在启动前设置 `ADMIN_USERS="用户名:密码"` 和 `JWT_SECRET="长随机字符串"`。
...
构建会先执行前端、Node 配置脚本和 Service Worker 的 TypeScript 检查；Go 服务端用 `cd server-go && go test ./...` 单独验证。构建输出的文件位于 `dist/` 目录下，可将其部署至任何静态文件服务器（如 Caddy、GitHub Pages、Netlify 等）。
...
> profile 内的 `baseUrl` / `apiKey` 字段会被静默丢弃 —— 同步请求一律走团队 API 代理，对应的真实上游由部署侧 env 按 `UPSTREAM_MODE` 决定。
