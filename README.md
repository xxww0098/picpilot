...
> 💡 **Go 内置 reverse 模式**：设置 `UPSTREAM_MODE=reverse`、`REVERSE_PROXY_URL=internal`，管理员在「管理面板 → 逆向账号」导入 ChatGPT/Codex OAuth JSON。账号只存入 PicPilot SQLite 数据库，不需要给 auth 容器挂载账号目录；如仍需外部 chatgpt2api，可把 `REVERSE_PROXY_URL` 改回它的 `/v1` 地址并填写 Bearer key。
